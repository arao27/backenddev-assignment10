...
app.get('/api/tasks', requireAuth, async (req, res) => {
    try {
        ...
        res.json({ message: 'Tasks retrieved successfully', tasks, total: tasks.length });
    } catch (error) {
...
app.post('/api/tasks', requireAuth, async (req, res) => {
    try {
        ...
        const newTask = await Task.create({ title, description, priority, userId: req.user.id, completed: false });
